      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {topics.map((topic, index) => (
            <div
              key={topic.title}
              className="text-center space-y-3 sm:space-y-4 p-4 sm:p-6 rounded-lg hover:bg-lunara-silver/5 transition-all duration-300 cursor-pointer group min-h-[120px] flex flex-col justify-center"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="flex justify-center">
                <topic.icon
                  size={28}
                  className="text-muted-stardust group-hover:text-lunara-accent transition-colors duration-300 sm:w-8 sm:h-8"
                />
              </div>
              <h3 className="text-lg sm:text-xl font-garamond font-medium text-pearl-mist">
                {topic.title}
              </h3>
